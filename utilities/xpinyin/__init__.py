...
def findDataPath(path):
    realPath = os.path.join(path, 'Mandarin.dat')
    # print(realPath)
    if os.path.exists(realPath):
        # print(os.path.abspath(realPath))
        return os.path.abspath(realPath)
    else:
        return findDataPath(os.path.join(path, ".."))
...
class Pinyin(object):
    ...
    data_path = findDataPath(os.getcwd())
        ...
        try:
            with open(data_path) as f:
                for line in f:
                    k, v = line.split('\t')
                    self.dict[k] = v
        except FileExistsError and FileNotFoundError as e:
            print(e)
